...
# Loggable decorator
def loggable(file_name=DEFAULT_LOG_FILE_NAME, level=DEFAULT_LOG_LEVEL):
    def outer(func):
        def wrapper(*args, **kwargs):
            with Logger(file_name, level) as log:
                log.info(f'Method {func.__name__} started.')
                try:
                    result = func(*args, **kwargs)
                    log.info(f'Method {func.__name__} ended.')
                    return result
                except Exception as err:
                    log.error(f'Error occuted: {err}')
        return wrapper
    return outer
